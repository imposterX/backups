...
def import_data(update, context):
    ...
    if msg.reply_to_message and msg.reply_to_message.document:
        ...
        if filetype.split('.')[-1] not in ("backup", "json", "txt"):
            send_message(update.effective_message, tl(
                update.effective_message, "File cadangan tidak valid!"))
            return
        try:
        ...
        try:
            ...
            if data.get('bot_base') == "Emilia":
                    ...
                    for x in data['filters'].get('filters'):
                        ...
                        if is_self:
                            ...
                            filtersql.add_filter(
                                chat_id, x['name'], note_data, is_sticker, is_document, is_image, is_audio, is_voice, is_video, buttons)
                            imp_filters_count += 1
                        elif is_emilia:
                            is_sticker = False
                            is_document = False
                            ...
                            if x['has_markdown']:
                                note_data, buttons = button_markdown_parser(
                                    x['reply'], entities=0)
                                filtersql.add_filter(
                                    chat_id, x['name'], note_data, False, False, False, False, False, False, buttons)
                                imp_filters_count += 1
                            else:
                                NOT_IMPORTED += "- {}\n".format(x['name'])
                                NOT_IMPORTED_INT += 1
                ...
                if data['greetings'].get('security'):
                    ...
                    if not timeout_mode:
                        timeout_mode = 1
                    welcsql.set_welcome_security(chat_id, extra_verify, bool(secenable), str(
                        sectime), str(timeout), int(timeout_mode), str(secbtn))
                    imp_greet_pref = True
                ...
                if NOT_IMPORTED_INT:
                    ...
                    f.close()
                    context.bot.sendDocument(chat_id, document=open('{}-notimported.txt'.format(chat_id), 'rb'), caption=tl(
                        update.effective_message, "*Data yang tidak dapat di import*"), timeout=360, parse_mode=ParseMode.MARKDOWN)
        ...
        try:
            ...
            if data.get('bot_id') == 609517172:
                ...
                if data.get('data'):
                    ...
                    if data['data'].get('greetings'):
                        ...
                        if data['data']['greetings'].get('mute_text'):
                            ...
                            welcsql.set_welcome_security(
                                chat_id, getcur, extra_verify, cur_value, timeout, timeout_mode, data['data']['greetings'].get('mute_text'))
                        imp_greet_pref = True
                    ...
                    if conn:
                        text = tl(
                            update.effective_message, "Cadangan sepenuhnya dikembalikan pada *{}*. Selamat datang kembali! 😀").format(chat_name)
                    else:
                        text = tl(update.effective_message, "Cadangan sepenuhnya dikembalikan. Selamat datang kembali! 😀").format(
                            chat_name)
                    text += tl(update.effective_message,
                               "\n\nYang saya kembalikan:\n")
                    if imp_antiflood:
        ...
        try:
            ...
            LOGGER.exception("Impor untuk id chat %s dengan nama %s gagal.", str(
                chat_id), str(chat.title))
            return
...
def export_data(update, context):
    ...
    antiflood = {'flood_mode': flood_mode,
                 'flood_duration': flood_duration, 'flood_limit': flood_limit}
    ...
    blacklist_mode, blacklist_duration = blacklistsql.get_blacklist_setting(
        chat.id)
    blacklists = {'blacklist_mode': blacklist_mode,
                  'blacklist_duration': blacklist_duration, 'blacklists': all_blacklisted}
    ...
    blsticker_mode, blsticker_duration = blacklistsql.get_blacklist_setting(
        chat.id)
    blstickers = {'blsticker_mode': blsticker_mode,
                  'blsticker_duration': blsticker_duration, 'blstickers': all_blsticker}
    ...
    for x in all_filters:
        ...
        if filt.is_sticker:
            ...
            filt_type = 7
        filters_gen.append(
            {"name": x, "reply": filter_text, "type": filt_type})
    ...
    greetings["welcome"] = {"enable": pref, "text": welcome_m,
                            "content": cust_content, "type": welcome_type}
    ...
    greetings["goodbye"] = {"enable": pref, "text": goodbye_m,
                            "content": cust_content, "type": goodbye_type}
    ...
    greetings["security"] = {"enable": getcur, "text": cust_text, "time": cur_value,
                             "extra_verify": extra_verify, "timeout": timeout, "timeout_mode": timeout_mode}
    ...
    locks = {'lock_warn': lock_warn,
             'locks': locked_lock, 'restrict': locked_restr}
    ...
    warns = {"warn_limit": warn_limit, "warn_mode": warn_mode,
             "warn_filters": all_warn_filter, "chat_warns": allwarns}

    # Parsing backups
    backup = {"bot_id": bot_id, "bot_base": bot_base, "antiflood": antiflood, "blacklists": blacklists, "blstickers": blstickers, "disabled": disabled, "filters": filters,
              "greetings": greetings, "language": language, "locks": locks, "notes": notes, "report": report, "rules": rules, "warns": warns, "version": backup_ver}
    ...
    try:
        context.bot.sendMessage(TEMPORARY_DATA, "*Berhasil mencadangan untuk:*\nNama chat: `{}`\nID chat: `{}`\nPada: `{}`".format(
            chat.title, chat_id, tgl), parse_mode=ParseMode.MARKDOWN)
    except BadRequest:
        pass
    send = context.bot.sendDocument(current_chat_id, document=open('{}-Emilia.backup'.format(chat_id), 'rb'), caption=tl(update.effective_message,
...
def put_chat(chat_id, user_id, value, chat_data):
    ...
    if value == False:
        ...
        status = True
    chat_data[chat_id] = {'backups': {
        "status": status, "user": user_id, "value": value}}
